...
class AlienGo_CAMS:
    def __init__(self, name, conf):
        ...
        self.check_feasible = casadi.Function('check_ICS_feasible', [cxt], [self.check_ICS_feasible(cxt)])
        self.check = casadi.Function('check', [cxt, cu], [self.check_friction_cone(cxt,cu)])
        
        self.p_ee_jax_wrapped = wrap_convert(self.p_ee)
        self.cost_tau_func_wrapped = wrap_convert(self.cost_tau)
        self.dynamics_tau_func_wrapped = wrap_convert(self.x_next_tau)
        self.check_feasible_wrapped = wrap_convert(self.check_feasible)    
        self.check_wrapped = wrap_convert(self.check)
    ...
    def check_friction_cone(self, x, u):
        _, a, _ = u[:4], (u[4] + 0.5), (u[5] + self.conf.dt_delta)
        ...
        cop = xy_foot_0 + a * (xy_foot_1 - xy_foot_0)
        ddx_CoM = self.conf.w**2 * (x_CoM - cop)
        f_0x = beta*self.conf.m * ddx_CoM[0]
        f_1x = (1 - beta)*self.conf.m * ddx_CoM[0]
        f_0y = gamma*self.conf.m * ddx_CoM[1]
        f_1y = (1 - gamma)*self.conf.m * ddx_CoM[1]
        aa = casadi.fmax(0,(f_0x**2 + f_0y**2)**0.5 - self.conf.mu*a*self.conf.m*self.conf.g)
        bb = casadi.fmax(0,(f_1x**2 + f_1y**2)**0.5 - self.conf.mu*(1-a)*self.conf.m*self.conf.g)
        
        inner_steps = 1
        x_next = x
        u_xnext= casadi.vertcat(u[:-1], u[-1]/inner_steps)
        for _ in range(inner_steps):
            x_next = self.simulate_fun(x_next, u_xnext)
            x_CoM_next = x_next[4:6]

            ddx_CoM = self.conf.w**2 * (x_CoM_next - cop)
            f_0x = beta*self.conf.m * ddx_CoM[0]
            f_1x = (1 - beta)*self.conf.m * ddx_CoM[0]
            f_0y = gamma*self.conf.m * ddx_CoM[1]
            f_1y = (1 - gamma)*self.conf.m * ddx_CoM[1]
